...
Channel::Channel() : _channelname("Unknown"), _topic_message("None") \
					 ...
					 _string_passphrase(""), _userlimit(-1) {}
...
//when using find(), the ==operator must be overloaded to wirk with client classes aswell
int Channel::rm_priv(class &Client cli) {
...
//when using find(), the ==operator must be overloaded to wirk with client classes aswell
int Channel::add_user(class &Client cli) {
...
//setter
void Channel::set_users(std::map<std::string, t_info> users) {_users = users;}
...
void Channel::set_userlimit(int limit) {_user_limited = limit;}

//getter
std::map<std::string, t_info> Channel::get_users(void) {return (_users);}
